...
#' @description The function, for a given similarity matrix, will separate the data using a spectral space. It does not normalize the Laplacian matrix compared to other algorithms
...
#'      xlim=c(-1,1),ylim=c(-1,1)); points(0,0,pch='+');
#' plot(res$eigenVal, main="Laplacian eigenvalues",pch='+'); 
UnormalizedSC <- function(W, K=5, flagDiagZero=FALSE, verbose = FALSE){
  ...
  V <- U$vectors[ , ((ncol(L)-K+1):ncol(L))]
  if(verbose){print(V)}
  
  #K-means clustering
  if(verbose){message("CUTTING THE CLUSTER WITH KMEANS")}
  cluster <- kmeans(V, K)$cluster
